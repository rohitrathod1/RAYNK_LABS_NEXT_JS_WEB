...
import { TEAM_SECTIONS } from "../constants";

export function HeroSection({ data }: { data: { title: string; subtitle: string; backgroundImage: string } }) {
  const imageSrc = useMemo(() => resolveImageSrc(data.backgroundImage), [data.backgroundImage]);
  const [pointer, setPointer] = useState({ x: 50, y: 45 });

  return (
    <section
      id={TEAM_SECTIONS.hero}
    ...
    >
      ...
      <motion.div
        aria-hidden="true"
        className="absolute -left-24 top-20 h-60 w-60 rounded-full bg-blue-500/18 blur-3xl"
        animate={{ x: [0, 36, 0], y: [0, -16, 0], opacity: [0.45, 0.8, 0.45] }}
        transition={{ duration: 10, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
      />
      <motion.div
        aria-hidden="true"
        className="absolute right-[-8%] top-[22%] h-72 w-72 rounded-full bg-cyan-400/12 blur-3xl"
        animate={{ x: [0, -30, 0], y: [0, 22, 0], opacity: [0.35, 0.65, 0.35] }}
      ...
      <div
        ...
        style={{
          background: `radial-gradient(420px circle at ${pointer.x}% ${pointer.y}%, rgba(59,130,246,0.18), transparent 42%)`,
        }}
      />
      ...
      <div className="section-container relative z-10 flex w-full items-center px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24 2xl:max-w-screen-2xl 2xl:px-20 2xl:py-32">
        ...
        >
          ...
          <motion.div variants={blurReveal} className="mt-6 max-w-2xl text-balance text-4xl font-black leading-[0.95] text-white sm:mt-8 sm:text-5xl md:text-6xl lg:text-7xl 2xl:text-8xl">
            <SplitWords text={data.title} inheritParent />
          </motion.div>
          ...
          <motion.div variants={heroItem} className="mt-8 flex flex-col items-start gap-4 sm:flex-row sm:flex-wrap 2xl:mt-10">
            ...
            <Button asChild size="lg" variant="outline" className="group h-12 rounded-full border-white/14 bg-white/6 px-6 text-sm text-white backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/50 hover:bg-white/10 hover:text-white sm:h-14 sm:px-7 sm:text-base">
              <Link href={`#${TEAM_SECTIONS.join}`}>
                Join Us
                <BriefcaseBusiness className="ml-2 h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
              </Link>
            </Button>
          </motion.div>

          <motion.div variants={heroItem} className="mt-8 flex flex-wrap items-center gap-3 text-sm text-white/55 2xl:mt-10 2xl:text-base">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 backdrop-blur">
              <Sparkles className="h-3.5 w-3.5 text-primary" aria-hidden="true" />
              Product-minded team culture
            </span>
            <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 backdrop-blur">
              <Sparkles className="h-3.5 w-3.5 text-primary" aria-hidden="true" />
              Design to delivery range
            </span>
          </motion.div>
        </motion.div>
      </div>
...
export function HeroSectionSkeleton() {
  return (
    <section className="relative flex min-h-[72vh] animate-pulse items-center overflow-hidden bg-[#06070b] sm:min-h-[78vh] lg:min-h-[88vh] 2xl:min-h-[92vh]">
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(7,10,18,0.88),rgba(6,7,11,0.96))]" />
      <div className="section-container relative z-10 flex w-full items-center px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24 2xl:max-w-screen-2xl 2xl:px-20 2xl:py-32">
        <div className="max-w-3xl">
          <div className="h-10 w-60 rounded-full bg-white/10 sm:w-72" />
          ...
          <div className="mt-8 flex gap-4">
            <div className="h-12 w-40 rounded-full bg-white/10 sm:h-14 sm:w-44" />
            <div className="h-12 w-32 rounded-full bg-white/10 sm:h-14 sm:w-36" />
          </div>
        </div>
      </div>
    </section>
